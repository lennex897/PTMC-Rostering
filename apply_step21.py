...
new_eligibility = '''def is_person_unavailable(
    ...
    """
    Any plotted availability entry blocks automatic scheduling regardless
    of its code. Manual locked assignments remain authoritative because
...
if old_eligibility not in eligibility:
    raise SystemExit(
        "Could not find is_person_unavailable() in eligibility.py."
    )
...
if old_cover not in cover:
    raise SystemExit(
        "Could not find cover availability check in cover_scheduler.py."
    )
...
for path, content in (
    ...
    backup = path.with_suffix(".py.step21_backup")
    if not backup.exists():
        backup.write_text(path.read_text())
    path.write_text(content)

print("Step 21 applied successfully.")
