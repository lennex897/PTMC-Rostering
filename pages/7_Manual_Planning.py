...
st.caption(
    "Lock selected duties, manual reserves and covers, or record duty interest before roster generation."
)
...
duty_tab, reserve_tab, cover_tab, interest_tab, review_tab = st.tabs(
    [
        "Manual duties",
        "Manual reserves",
        "Manual covers",
...
with reserve_tab:
    st.subheader("Lock a manual reserve")
    st.caption(
        "Reserve staffing is manual-only. A plotted reserve receives 0 points "
        "unless separately activated and is protected from automatic duties "
        "and covers on the same date."
    )

    r1, r2, r3 = st.columns([1.1, 1.7, 1])
    ...
    with r1:
        reserve_date = st.date_input(
            ...
            max_value=selected_month_end,
            format="DD/MM/YYYY",
            key="manual_reserve_date",
        )

    reserve_person_options = personnel_available_on(
        person_by_name,
        reserve_date,
    )

    with r2:
        reserve_person = st.selectbox(
            "Personnel",
            reserve_person_options,
            key=f"manual_reserve_person_{reserve_date.isoformat()}",
            disabled=not reserve_person_options,
        )

    with r3:
        reserve_centre = st.selectbox(
            "Reserve centre",
            CENTRES,
            key="manual_reserve_centre",
        )

    reserve_remarks = st.text_input(
        "Remarks",
        key="manual_reserve_remarks",
    )

    reserve_availability_code = (
        get_availability_code(
            availability_rows,
            reserve_person,
            reserve_date,
    ...
    existing_reserve_day = (
        [
            row
            for row in manual_assignments
            if (
                row.personnel_name == reserve_person
                and row.assignment_date == reserve_date
            )
        ]
        if reserve_person
        else []
    ...
    if reserve_availability_code:
        st.warning(
            f"{reserve_person} has availability code "
            f"{reserve_availability_code} on {reserve_date:%d %b %Y}."
        )

    if existing_reserve_day:
        st.warning(
            f"{reserve_person} already has "
            f"{len(existing_reserve_day)} locked assignment(s) on this date."
        )
    ...
    if st.button(
        ...
        type="primary",
        use_container_width=True,
        key="lock_manual_reserve_button",
        disabled=not reserve_person_options,
    ):
        payload = {
            "roster_month_id": roster_month_id,
            "personnel_name": reserve_person,
            "assignment_date": reserve_date.isoformat(),
            "assignment_kind": "RESERVE",
            "centre": reserve_centre,
            ...
            "remarks": reserve_remarks.strip() or None,
        }
        ...
        try:
            ...
            st.error(
                f"Unable to save manual reserve: {exc}"
            )
        else:
            clear_manual_cache()
            st.success("Reserve locked.")
            st.rerun()
    ...
    if not manual_assignments:
        ...
        for row in manual_assignments:
            if row.assignment_kind == "DUTY":
                details = row.qualified_role or ""
            elif row.assignment_kind == "RESERVE":
                details = f"{row.centre or ''} RESERVE".strip()
            else:
                details = row.cover_label or row.assignment_kind
        ...
        elif selected_assignment.assignment_kind == "RESERVE":
            edit_reserve_centre = st.selectbox(
                "Reserve centre",
                CENTRES,
                index=CENTRES.index(
                    selected_assignment.centre
                ...
                key=f"edit_assignment_reserve_centre_{selected_assignment_id}",
            )

            edit_payload.update({
                "assignment_kind": "RESERVE",
                "centre": edit_reserve_centre,
                "role_name": RESERVE_ROLE,
                "session": "FULL_DAY",
                "cover_requirement_id": None,
                "cover_label": None,
        ...
        if selected_assignment.assignment_kind not in {"COVER", "COVER_RESERVE"}:
            matching_requirements = []
